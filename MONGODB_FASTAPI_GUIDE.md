# MongoDB and FastAPI Integration Guide - AI Email Classification System

## Overview

This guide demonstrates how to use MongoDB and Motor (async driver) in FastAPI applications to build high-performance real-time email classification systems.
...
### Technology Stack
- **Database**: MongoDB 7.0 (document database suitable for unstructured email data)
...
- ✅ **Native Async Support**: Motor perfectly integrates with FastAPI's async model
...
class MongoDBService:
    ...
    async def connect(self) -> None:
        ...
        - minPoolSize: Ensures minimum hot connections available
    ...
    async def _create_indexes(self) -> None:
        ...
        # Compound index (descending date queries)
...
async def save_classified_email(self, email_data: ClassifiedEmail) -> str:
    ...
    Workflow:
        ...
        2. Use upsert operation (allow updating existing emails)
    ...
    try:
        ...
        logger.info(
            f"Saved classified email: {email_data.email_id} "
...
async def get_classified_emails_by_label(
    ...
    try:
        ...
        logger.debug(
            f"Retrieved {len(results)} emails with "
            f"label '{classification_label}'"
        )
...
class ClassifiedEmail(BaseModel):
    ...
    body_text: str = Field(..., description="Email body")
    ...
    sentiment_analysis: Optional[SentimentAnalysis] = Field(
        None, 
        description="Sentiment and urgency analysis"
    )
...
async def classify_and_save_email(email_data: EmailData):
    ...
    """
    try:
        # 1. AI classification processing
...
services:
  ...
  mongodb:
    ...
    volumes:
      ...
      - mongodb_config:/data/configdb  # Config volume
      - mongodb_logs:/var/log/mongodb  # Log volume
    ...
    # Health Check (ensure successful container startup)
    healthcheck:
...
### 1. Complete Email Classification Workflow
...
async def get_work_emails(limit: int = 50):
    """Get all emails classified as 'work'"""
...
{
    "maxPoolSize": 50,           # ✓ Large enough to handle concurrent requests
    "minPoolSize": 10,           # ✓ Maintain hot connections, avoid cold starts
...
await collection.create_index([("created_at", -1)])      # Date range queries
await collection.create_index("classification_label")    # Classification filtering
await collection.create_index("sender")                  # Sender queries
...
print(result)  # Check if index is being used
...
- Production-level connection pool management
- Optimized indexing strategy
- Comprehensive error handling
- Containerized Docker deployment

💡 **Key Points**:
1. Use Motor async driver to ensure non-blocking operations
2. Configure connection pool size appropriately
3. Create indexes for frequently queried fields
4. Use TTL indexes for automatic cleanup of expired data
